...
//$cm = get_coursemodule_from_id('protects', $cmid, 0, false, MUST_EXIST);
//$course = $DB->get_record('course', array('id' => $cm->course), '*', MUST_EXIST);
//require_login($course, true, $cm);
//$PAGE->set_url('/mod/copysafeweb/function.php', array('id' => $cm->id));
//$PAGE->set_title('My modules page title');
//$PAGE->set_heading('My modules page heading');

$fucname= optional_param('fucname','', PARAM_TEXT);

	if( $fucname == "file_upload" ){
                $err = required_param('error', PARAM_INT);
		$msg = wpcsw_file_upload($err) ;
	...
	if( $fucname == "file_search" ){
               $srch= required_param('search', PARAM_TEXT);
               $postid = required_param('post_id', PARAM_INT);
	       $data = wpcsw_file_search($srch,$postid) ;
		echo $data ;
	}
	
	if( $fucname == "setting_save" ){
                $nname= required_param('nname', PARAM_TEXT);
                $postid = required_param('post_id', PARAM_INT);
                $set_data = required_param('set_data', PARAM_TEXT);
		$data = wpcsw_setting_save($nname,$postid,$set_data) ;
		echo $data ;
	}
        
        if( $fucname == "setting_del" ){
                $nname= required_param('nname', PARAM_TEXT);
		$data = wpcsw_setting_del($nname) ;
		echo $data ;
	}
	
	if( $fucname == "get_parameters" ){
                $finame = required_param('filename', PARAM_TEXT);
		$data = wpcsw_get_parameters($finame) ;
		echo $data ;
	}
        
        if( $fucname == "get_config" ){
		$data = wpcsw_get_config() ;
		echo $data ;
	}
...
function wpcsw_get_parameters($finame){
     global $DB,$CFG;
	//$postid = $_POST["post_id"] ;
	$filename = trim($finame) ;
...
function wpcsw_file_upload($err){
	$file_error 	= $err ;  
...
function wpcsw_file_search($srch,$postid){
	// get selected file details
	if (@!empty($srch) || @!empty($postid)) {
...
function wpcsw_setting_save($nname,$postid,$set_data){
    global $DB,$CFG;
	$postid = $postid ;
	$name = trim($nname) ;
	$data = (array)json_decode(stripcslashes($set_data)) ;
...
function wpcsw_setting_del($nname){
    global $DB,$CFG;
 $name = trim($nname) ;
 $result=$DB->delete_records('copysafeweb_settings',array('name'=>$name)); 
 if($result)
 unlink($CFG->dirroot.'/mod/copysafeweb/copysafewebimage/'.$name);
 return $result ;
